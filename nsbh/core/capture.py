...
def caprate_no(mx, sigxn, rho=Const.LocalDM, vbar=Const.LocalVecDisp):
    """
...
def caprate(mx, sigxn, rho=Const.LocalDM, vbar=Const.LocalVecDisp):
    """
    ...
    if mx < 1e-6:
        ...
        return caprate_no(mx,sigxn,rho,vbar)


def numx(age, mx, sigxn, rho=Const.LocalDM, vbar=Const.LocalVecDisp, tau=None):
    """
    ...
    """
    if tau is None:
        return caprate(mx,sigxn,rho,vbar)*age
    else:
        return caprate(mx,sigxn,rho,vbar)*tau*(1 - np.exp(-age/tau))